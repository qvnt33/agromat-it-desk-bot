...
Бот синхронізує YouTrack із Telegram: нові задачі автоматично зʼявляються у чаті з кнопкою "Прийняти", інженер може призначити себе в один клік, а зміни статусу та виконавця підтягуються, доки заявка не перейде в архів.
...
2. FastAPI-бекенд формує повідомлення, зберігає звʼязок `issue_id ↔ message_id` у MySQL та публікує запис у Telegram.
3. **Кнопка "Прийняти"**:
...
- Токени **не зберігаються у відкритому вигляді**. Реалізація:
  1. розрахунок SHA-256 для перевірки, чи змінився токен;
  2. шифрування токен ключем `SHA-256(USER_TOKEN_SECRET)` і збереження результата у поле `token_encrypted`.
...
| `USER_TOKEN_SECRET` | ключ для шифрування персональних токенів (обовʼязково налаштувати)|
| `MYSQL_HOST` / `MYSQL_PORT` | хост/порт MySQL (default: `localhost:3306`)               |
| `MYSQL_DATABASE`    | назва бази (default: `support`)                                   |
| `MYSQL_USER` / `MYSQL_PASSWORD` | облікові дані для підключення                          |
| `MYSQL_CHARSET`     | кодування зʼєднання (default: `utf8mb4`)                          |
| `DATABASE_BACKEND`  | `mysql` або `sqlite` (для тестів/відладки; default: `mysql`)      |
...
## Обмеження та нюанси
- Один YouTrack-акаунт (`yt_user_id`) можна привʼязати тільки до одного Telegram. Щоб змінити чат – зробіть `/unlink`.
