...
## Синхронізація YouTrack ↔ Бекенд ↔ Telegram

### Створення заявки

Коли користувач створює нову задачу в YouTrack:
- Спрацьовує скрипт `yt2tg_create.js`, який надсилає повний payload (ID, тема, опис, автор, статус, виконавець) у бекенд через `POST /youtrack`.
- Бекенд FastAPI отримує цей вебхук, очищує HTML-опис, формує повідомлення й надсилає його в Telegram через Aiogram.
- Повідомлення зберігається у SQLite разом із ID задачі (звʼязок **issue_id ↔ message_id**).
- Користувач у Telegram бачить заявку з кнопкою "Прийняти".

### Прийняття задачі в Telegram

Коли користувач натискає "**Прийняти**":
- Бот перевіряє авторизацію (токен YouTrack).
- Бекенд через REST-API YouTrack оновлює поля "**Виконавець**" і "**Статус**".
- Це автоматично тригерить у YouTrack правило `auto-assign_on_new.js`, яке також:
- фіксує призначення поточного користувача (`ctx.currentUser`);
- надсилає у бекенд `/youtrack/update` з міткою changes: ['auto-assigned'] ￼.
- Бекенд отримує вебхук `/youtrack/update` і редагує відповідне повідомлення в Telegram — змінює статус, додає імʼя виконавця та прибирає кнопку.

### Оновлення заявки в YouTrack

Коли в YouTrack змінюється заявка (статус, опис, виконавець тощо):
- Спрацьовує скрипт `yt2tg_update.js`.
- Він перевіряє, чи змінилось щось зі списку полів.
- Якщо так, надсилає у бекенд `/youtrack/update`.
- Якщо зміна — лише перехід зі "**Нова**", вона ігнорується (щоб не дублювати створення) ￼.
- Бекенд отримує цей "ping" і сам повторно запитує актуальні дані заявки через API YouTrack.
- Потім оновлює повідомлення у Telegram, гарантуючи, що дані залишаються синхронізованими навіть після ручних змін у YouTrack.
- Якщо повідомлення із заявкою в Telegram знаходиться **>48 годин** – оновлення ігнорується.

## Вебхуки від YouTrack

Бот отримує два типи вебхуків від YouTrack Automation App:

- `/youtrack` — надсилається при створенні нової заявки.

    - Бекенд форматує дані (тема, опис, автор, статус, виконавець) і надсилає повідомлення у Telegram із кнопкою "**Прийняти**".
    - Також створюється звʼязок `issue_id ↔ message_id` у базі даних для подальших оновлень.

- `/youtrack/update` — надсилається при зміні **статусу** або **виконавця** у YouTrack.

    - Бекенд знаходить відповідне повідомлення в Telegram і **редагує його в реальному часі**, щоб відобразити актуальний стан заявки без дублювання.
